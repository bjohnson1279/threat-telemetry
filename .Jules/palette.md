...
## 2026-10-25 - Prevent focus loss on conditional UI elements
**Learning:** When a conditionally rendered interactive element (like a 'Clear search' button) is clicked and subsequently unmounts, keyboard users lose focus context, defaulting back to the start of the document (or document body), which ruins keyboard navigation.
**Action:** Always programmatically restore focus to a logical next element (like the associated input field) before or immediately after the element unmounts to maintain keyboard accessibility.
