...
## 2026-09-14 - Enforce array length limits on ingest payload parsing
**Vulnerability:** The `/api/v1/ingest` endpoint was parsing and mapping array payloads (especially CSV files) without validating the maximum number of records, creating a Denial of Service (DoS) and excessive database strain vector.
**Learning:** Even if the Express body parser limits the total request payload size in bytes (e.g., `10mb`), an attacker can pack hundreds of thousands of small records into a single payload, triggering out-of-memory errors or starving the database connections during mapping and insertion loops.
**Prevention:** Always enforce a logical item count limit (e.g., maximum array length) in addition to byte size limits for bulk ingestion API endpoints to cap processing overhead.
